...
class hmc5883l:
	...
	def calibrate(self): 
		...
		maxx = x 
		minx = x # Set initial values to current magnetometer readings.
		maxy = y
		miny = y
		...
		while(cal):
			...
			secmillis  = (time.time()*1000)-calTimeWas
			secs = (int((10000-secmillis+1000)/1000))
	...
	def heading(self,xOFF,yOFF):
		(x, y, z) = self.axes()
		headingRad = math.atan2(y-yOFF, x-xOFF)
...
if __name__ == "__main__":
	...
	compass = hmc5883l(gauss = 4.7, declination = (+6,11))
	x,y=compass.calibrate()
	print(x,y)

	while True:
		sys.stdout.write("\rHeading: " + str(compass.degrees(compass.heading(x,y))) + "     ")
		sys.stdout.flush()
		time.sleep(0.5)
